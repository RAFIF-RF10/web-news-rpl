...
export const Dashboard = () => {
  const { getUser, getAllUsers } = useAuth();
  ...
  const handleApprove = async (id, status) => {
    ...
    if (result.success) {
      Swal.fire(
        "Berhasil",
        `Berita ${status === "published" ? "diapprove" : "direject"}!`,
        "success"
      );
      refetch();
    } else {
  ...
  const stats = {
    ...
    activeUsers: users.length, // nanti bisa diganti kalau ada API
  };

  return (
    <div className="flex w-full bg-[#ECEEF0] dark:bg-[#252E4B]">
      <div className="flex-1 overflow-auto">
        ...
        <div className="p-8 mt-8 space-y-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            <div className="bg-[#5A77DF] dark:bg-[#2E3B5B] rounded-xl shadow-md p-6 border border-blue-100 dark:border-blue-500/30">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-medium text-white dark:text-gray-300">
                    Total Berita
                  </p>
                  <h3 className="text-3xl font-bold text-gray-100 dark:text-gray-100 mt-1">
                    {stats.totalNews}
                  </h3>
                </div>
            ...
            <div className="bg-[#5A77DF] dark:bg-[#2E3B5B] rounded-xl shadow-md p-6 border border-orange-100 dark:border-orange-500/30">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-medium text-white dark:text-gray-300">
                    Active Users
                  </p>
                  <h3 className="text-3xl font-bold text-white dark:text-gray-100 mt-1">
                    {stats.activeUsers}
                  </h3>
                </div>
                <div className="bg-blue-100 dark:bg-blue-500/30 p-3 rounded-full">
                ...
                <div className="overflow-x-auto max-h-96 overflow-y-auto border border-gray-200 dark:border-gray-600 rounded-lg">
                  <table className="w-full min-w-full">
                    <thead className="bg-gray-50 dark:bg-[#3B4672] sticky top-0 z-10">
                      <tr>
                        <th className="px-4 py-3 text-left text-xs font-semibold text-gray-500 dark:text-gray-200 uppercase">
                          No
                        </th>
                        <th className="px-6 py-3 text-left text-xs font-semibold text-gray-500 dark:text-gray-200 uppercase min-w-[200px]">
                          Judul
                        </th>
                        <th className="px-6 py-3 text-left text-xs font-semibold text-gray-500 dark:text-gray-200 uppercase">
                          Tanggal
                        </th>
                        <th className="px-6 py-3 text-left text-xs font-semibold text-gray-500 dark:text-gray-200 uppercase">
                          Status
                        </th>
                        <th className="px-6 py-3 text-left text-xs font-semibold text-gray-500 dark:text-gray-200 uppercase min-w-[150px]">
                          Actions
                        </th>
                      </tr>
                    </thead>
                    <tbody className="bg-white dark:bg-[#1E253A] divide-y divide-gray-200 dark:divide-gray-600">
                      {newsData?.map((n, index) => (
                        <tr
                          key={n.id}
                          className="hover:bg-gray-50 dark:hover:bg-[#2C3560]"
                        >
